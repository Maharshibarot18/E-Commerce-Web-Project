...
// Creating upload Endpoint for multiple images
app.use('/images', express.static('upload/Images'));
app.post("/upload", upload.array('product', 4), (req, res) => {
    const fileNames = req.files.map(file => `http://localhost:${port}/images/${file.filename}`);
    res.json({
        success: 1,
...
const Product = mongoose.model("Product", {
    ...
    images: {
        type: [String], // Change to an array of strings to store multiple URLs
        required: true,
    },
    category: {
        type: String,
